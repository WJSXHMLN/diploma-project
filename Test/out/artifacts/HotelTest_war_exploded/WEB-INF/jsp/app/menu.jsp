...
<script type="application/x-javascript"> 
	...
	function addShopCar(foodId){
		...
		//加入商品到购物车需要条件，加入哪一个商品foodID，到哪一个餐桌的购物车dinnerTableId
	...
	function blurFn(obj,foodId,buyNum,dinnerTableId){
		...
		//如果用户输入的数量<1 或者 不是一个数字，        就赋值为原来的购买数量buyNum
		if(num < 1 || isNaN(num)){
			...
			window.location.href = "${pageContext.request.contextPath}/app/shopCar.action?method=update&foodId="+foodId+"&buyN		um="+Math.ceil(num)+"&dinnerTableId="+dinnerTableId;
		}
	}
	...
	//占位和取消占位
	<!-- 第一个参数：餐桌的id   第二个参数：更改对应table_status字段的值 0或者1  -->
						...
						<div class="collapse navbar-collapse" id="bs-megadropdown-tabs">
							<ul class="nav navbar-nav navbar-right">
								...
								<c:if test="${empty session_user }">
									...
									<li class="head-dpdn">
										<a href="${pageContext.request.contextPath}/app/register.do">注册</a>
									</li>
								</c:if>
								...
								<li class="head-dpdn">
									<a href="${pageContext.request.contextPath}/sys/login.do">个人中心</a>
								</li>
							</ul>
						</div>
					</nav>
