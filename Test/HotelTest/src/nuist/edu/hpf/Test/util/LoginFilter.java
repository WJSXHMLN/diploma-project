...
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		/*
		*  先看session中有没有 ，有则表示已经登录
		* 	session中没有， 看 cookie有没有
		* 						cookie中有，且有效，并把信息放入如session中，然后放行
		*
		*
		* */
		...
		if(user == null) {
			//先发现session 中没有用户信息
			//查看 cookie 中 是否有保存用户名和密码的cookie
			Cookie cookie = CookieUtils.getCookieByName( httpServletRequest,ConstantUtil.COOKIE_NAME);
			
			if(cookie != null) {
				//获取cookie的value值
				String value = URLDecoder.decode(cookie.getValue(), "utf-8");  //中文要解码  decode
				//分割用户和密码   （#  来分割）
				...
				//登录不成功
				request.setAttribute("message", "请先登录！");    //1 提示
				if(flag) {
					//前台请求
					//2 跳转到登录页面
					request.getRequestDispatcher("/WEB-INF/jsp/app/login.jsp").forward(request, response);
