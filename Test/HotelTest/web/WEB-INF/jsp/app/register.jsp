...
<script type="text/javascript">
	...
	function  formOnblur(id){
		//获取当前需要校验的输入框的值
		var  fieldValue = document.getElementById(id).value;  //fieldValue是用户输入的值
		document.getElementById(id+"_error").innerHTML="";    //清空当前提示框的错误信息，不然即使输入对了，还是会提示错误信息
		var flag = true;  //flag不符合要求返回false
		if(id == "loginName"){
			...
			if(fieldValue == null || fieldValue == ""){
				...
				$.ajax({
				   ...
				   //同步请求   (比如说loginName这边走不完，就不会走下面password及以后的代码)
				   async:false,  
	...
	function onRegister(){
		//思路：所有输入框输入完成后，在点击注册按钮之前 还会失去焦点，那么再调用一次失去焦点看看
		...
		for (var i = 0; i < ids.length; i++) {
			...
			if(!formOnblur(ids[i])){
				//不符合要求  							formOnblur(ids[i])返回false,取反则返回true
				return false;  //阻止默认行为
			}
		} 
	...
	<div class="login-page about">
		<div class="container"> 
			...
			<div class="login-agileinfo">
																														<%-- onsubmit="return  onRegister() 让返回值传给form --%>
				<form action="${pageContext.request.contextPath}/app/register.do" method="post" id="registerform"  onsubmit="return  onRegister()"> 
					...
					<input class="agile-ltext" type="text" id="phone" name="phone"  placeholder="请输入手机号"
